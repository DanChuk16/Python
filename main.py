...
try:
    ...
    for item in numbers:
        if item > 0:
            start_index = numbers.index(item)
            break
    for e in range(lengthOfFList-1, -1, -1):
        if numbers[e] > 0:
            end_index = e
            break
    partList_2 = numbers[start_index+1: end_index].copy()
    Sum = 0
    ...
    print(f'The sum of elements between the first and last positive element: {Sum}')
    print()
